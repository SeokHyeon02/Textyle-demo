...
app = FastAPI(title="TexTyle FashionCLIP Hybrid Search Server (v3, port 8003)")
...
class QueryIntent(BaseModel):
    reasoning: str = Field(default="", description="query analysis reasoning")
    color: str = Field(default="", description="target color, empty string if absent")
    color_mode: str = Field(default="ignore", description="target, same, different, or ignore")
    design: str = Field(default="", description="style, length, detail, or design phrase")
    description: str = Field(default="", description="long English search description of the target garment (used as the search query)")


class QueryAnalysisV3(BaseModel):
    reasoning: str = Field(description="query analysis reasoning")
    is_fashion: bool = Field(description="true if the image clearly shows a clothing/fashion item")
    color: str = Field(description="target color the user wants (canonical lowercase English), empty string if absent")
    color_mode: str = Field(description="target, same, different, or ignore")
    design: str = Field(description="short design keyword summary in English (e.g. 'oversized hoodie')")
    ...
    description: str = Field(description="long English description of the TARGET garment combining image design + requested change (60-120 words, used as the search query)")
...
async def analyze_query_v3(
    user_query: str,
    image_obj: Image.Image,
    rule_main_categories: list | None = None,
    rule_sub_categories: list | None = None,
) -> tuple[QueryIntent, list[str], list[str]]:
    """v3: 이미지+텍스트를 함께 Gemini에 보내 검색 description + 구조화 intent를 한 번에 추론.

    - description: 검색 벡터로 그대로 쓸 긴 영어 묘사 (이미지 디자인 + 텍스트 변경 반영)
    - color/color_mode/design/category: 리랭킹용 구조화 필드
    반환: (QueryIntent(description 포함), main_categories, sub_categories)
    """
    # ── 룰 기반 fallback 사전 계산 ────────────────────────────────────────────
    fallback_color = infer_color_from_text(user_query)
    fallback_color_mode = infer_color_mode(user_query)
    ...
    fallback_desc_color = fallback_color if fallback_color_mode in {"target", "different"} else ""
    fallback_description = " ".join(
        word for word in ["a photo of", fallback_desc_color, fallback_design, "clothing"] if word
    ).strip()
    fallback_intent = QueryIntent(
        reasoning="rule based fallback",
        color=fallback_color,
        ...
        description=fallback_description or "a photo of clothing",
    )
    fallback_intent = apply_excluded_color_phrase(user_query, fallback_intent)
    fallback_main = list(rule_main_categories or [])
    fallback_sub = list(rule_sub_categories or [])
    ...
    system_prompt = f"""\
You are a fashion search query generator for a Korean fashion app. You receive:
1. A user's Korean (or mixed) query about clothing (may be empty).
2. An image of a reference garment.

Produce a search query for a FashionCLIP retrieval system PLUS structured intent.

How to combine image + query:
- If the user asks to CHANGE something (e.g. "사진과 비슷한데 빨간색으로", "같은 디자인 검정으로",
  "와이드핏으로"), describe the IMAGE'S garment but apply the requested change (use the requested
  color/fit/detail instead of the image's original).
- If the user asks to keep it / wants the same / the query is empty, describe the image faithfully.
- If the user specifies a different category than the image, follow the user's category and use the
  image only as design/color reference.

Fields to return as JSON:
- reasoning: 1~2 English sentences.
- is_fashion: true if the image clearly shows a clothing/fashion item; false otherwise.
- color: target color the user explicitly wants (canonical lowercase English: black/white/gray/blue/red/
  green/yellow/brown/pink/purple/orange). "" if the user did not request a specific color.
- color_mode:
    - "target"    -> user wants a specific color (e.g. "검정 후드티 보여줘")
    - "same"      -> user wants the same color as the image (e.g. "같은 색으로")
    - "different" -> user wants a color different from the image (e.g. "색상이 다른 걸로")
    - "ignore"    -> color is not relevant
- design: short English keyword summary (e.g. "oversized hoodie zip-up").
- main_category: One of "상의", "하의", "아우터". "" if unclear.
- sub_category: closest match from hint list, or best inference. "" if truly unclear.
  Hint list: {_sub_cat_hint}
- description: ONE rich English search query for the TARGET garment. Start with "a photo of".
  Include the color word (use the requested color for target/different; the image's color for same/empty;
  no color word for ignore), garment type, fit & silhouette (oversized/relaxed/slim/cropped, shoulders,
  sleeves, length), material/fabric, and surface details (pockets, cuffs, hood, zipper, buttons,
  embroidery, prints, distressed, collar, panels). Pattern type only if non-solid. 60-120 words, natural
  flowing noun phrase, no preamble.
    ...
    try:
        ...
        response = gemini_client.models.generate_content(
            ...
            config=genai_types.GenerateContentConfig(
                system_instruction=system_prompt,
                response_mime_type="application/json",
                response_schema=QueryAnalysisV3,
            ),
        )
        if getattr(response, "parsed", None):
            parsed = response.parsed
        ...
        if not intent.description:
            intent.description = fallback_intent.description
        ...
        print(f"LLM v3 analysis failed, fallback used: {exc}")
        return fallback_intent, fallback_main, fallback_sub
...
def rerank_results(results, intent: QueryIntent, query_attrs, limit: int = 10, color_penalty_scale: float = 1.0, return_all: bool = False):
    color_mode = intent.color_mode if intent.color_mode in {"target", "same", "different", "ignore"} else "ignore"
    ...
    for item in results or []:
        ...
        # v3: 타겟 불일치(음수 색 보정) 페널티를 강화
        if color_adjustment < 0:
            color_adjustment *= color_penalty_scale
    ...
    if return_all:
        # v3: 슬라이스 없이 점수가 매겨진 전체 후보를 반환 (하드컷/이미지 가점은 엔드포인트에서)
        return [item for _, _score, item in reranked]
...
# ─────────────────────────────────────────────────────────────────────
# v3 STAGE 3: 이미지 벡터 2차 점수 유틸
# ─────────────────────────────────────────────────────────────────────
def _to_float_list(emb) -> list[float]:
    """DB vector 컬럼이 list 또는 JSON 문자열로 올 수 있으므로 통일."""
    if emb is None:
        return []
...
def fetch_candidate_image_embeddings(items) -> dict:
    """살아남은 후보의 fashion_embedding을 id(없으면 image_url)로 일괄 fetch.

    반환: { join_key: np.ndarray(512) }  (join_key는 id 우선, 없으면 image_url)
    """
    ids = [item.get("id") for item in items if item.get("id") is not None]
    urls = [item.get("image_url") for item in items if item.get("id") is None and item.get("image_url")]
    embeddings = {}
    ...
    try:
        if ids:
            resp = (
                ...
                .select("id, fashion_embedding")
                .in_("id", ids)
                .execute()
            )
            for row in resp.data or []:
                vec = _to_float_list(row.get("fashion_embedding"))
                if vec:
                    embeddings[("id", row.get("id"))] = np.asarray(vec, dtype=np.float32)
        if urls:
            resp = (
                supabase.table("clothes")
                .select("image_url, fashion_embedding")
                .in_("image_url", urls)
                .execute()
            )
            for row in resp.data or []:
                vec = _to_float_list(row.get("fashion_embedding"))
                if vec:
                    embeddings[("image_url", row.get("image_url"))] = np.asarray(vec, dtype=np.float32)
    except Exception as exc:
        print(f"[v3] candidate embedding fetch failed: {exc}")
        return {}

    return embeddings


def candidate_join_key(item):
    if item.get("id") is not None:
        return ("id", item.get("id"))
    if item.get("image_url"):
        return ("image_url", item.get("image_url"))
    return None
...
async def search_clothes(
    ...
    query: str = Form(None),
    image_weight: float = Form(0.5),          # w2: 이미지 유사도 가점 최대치 (min-max 정규화 후 곱)
    score_threshold: float = Form(0.15),      # 텍스트 리랭킹 점수 하드컷
    color_penalty_scale: float = Form(2.0),   # 색 불일치 페널티 배율 (v1 대비)
    change_gate: bool = Form(True),           # target/different 쿼리에서 w2를 0으로
):
    if not file:
    ...
    try:
        ...
        # v3: 이미지+텍스트를 함께 Gemini에 보내 description + 구조화 intent를 한 번에 추론
        intent, main_categories, sub_categories = await analyze_query_v3(
            query,
            image_obj,
        ...
        # v3: 검색 벡터 = Gemini description의 텍스트 단일 벡터 (이미지 가중합 제거)
        enhanced_query = (intent.description or "").strip() or "a photo of fashion item"
        is_specific_query = bool(
            intent.color or (intent.design or "").strip() or (intent.description or "").strip()
        )
        ...
        # ── 노브 clamp ──
        w2 = max(0.0, min(2.0, float(image_weight)))
        thr = float(score_threshold)
        cps = max(0.0, min(10.0, float(color_penalty_scale)))

        # ── STAGE 2: 리랭킹(전체 후보) → 텍스트 점수 하드컷 ──
        text_ranked = rerank_results(
            response.data, intent, query_attrs,
            color_penalty_scale=cps, return_all=True,
        )
        survivors = [
            it for it in text_ranked
            if float(it.get("_ranking", {}).get("final_score", 0.0)) >= thr
        ]
        hard_cut_fallback = False
        if not survivors and text_ranked:
            # threshold가 전부 잘라낸 경우: 빈 결과 대신 텍스트 점수 상위 10개로 폴백
            survivors = text_ranked[:10]
            hard_cut_fallback = True
            search_warnings = list(search_warnings) + ["score_threshold_cut_all_fallback_top10"]

        # ── STAGE 3: 이미지 벡터 2차 가점 + 결합 정렬 ──
        w2_eff = 0.0 if (change_gate and color_mode in {"target", "different"}) else w2
        image_stage_applied = w2_eff > 0 and bool(survivors)
        if image_stage_applied:
            q_vec = original_image_features.squeeze().detach().cpu().numpy().astype(np.float32)
            emb_map = fetch_candidate_image_embeddings(survivors)
            sims = []
            for it in survivors:
                key = candidate_join_key(it)
                cand = emb_map.get(key) if key else None
                sim = float(np.dot(q_vec, cand)) if cand is not None else None
                it["_ranking"]["image_sim"] = round(sim, 4) if sim is not None else None
                sims.append(sim)
            valid = [s for s in sims if s is not None]
            lo, hi = (min(valid), max(valid)) if valid else (0.0, 0.0)
            span = hi - lo
            for it, sim in zip(survivors, sims):
                if sim is None:
                    norm = 0.0
                elif span > 1e-9:
                    norm = (sim - lo) / span
                    ...
                    norm = 1.0
                text_score = float(it["_ranking"].get("final_score", 0.0))
                combined = text_score + w2_eff * norm
                it["_ranking"]["image_sim_norm"] = round(norm, 4)
                it["_ranking"]["w2_effective"] = round(w2_eff, 4)
                it["_ranking"]["final_score_combined"] = round(combined, 4)
                it["_combined_sort"] = combined
        else:
            for it in survivors:
                text_score = float(it["_ranking"].get("final_score", 0.0))
                it["_ranking"]["image_sim"] = None
                it["_ranking"]["image_sim_norm"] = None
                it["_ranking"]["w2_effective"] = 0.0
                it["_ranking"]["final_score_combined"] = round(text_score, 4)
                it["_combined_sort"] = text_score

        survivors.sort(key=lambda it: it.get("_combined_sort", 0.0), reverse=True)
        results = survivors[:10]
        for it in results:
            it.pop("_combined_sort", None)

        # ── v3 디버그 로그 ──
        print(
            "\n[FashionCLIP v3 Hybrid Debug]\n"
            f"original_query={query}\n"
            f"enhanced_query={enhanced_query}\n"
            f"color_mode={color_mode}, change_gate={change_gate}, w2={w2}, w2_eff={w2_eff}, "
            f"score_threshold={thr}, color_penalty_scale={cps}\n"
            f"main_categories={main_categories}, sub_categories={sub_categories}\n"
            f"raw_result_count={len(response.data or [])}, survivors_after_cut={len(survivors)}, "
            f"hard_cut_fallback={hard_cut_fallback}, image_stage_applied={image_stage_applied}"
        )
        for idx, it in enumerate(results, 1):
            r = it.get("_ranking", {})
            print(
                f"  [{idx}] {it.get('name')} | text={r.get('final_score')} "
                f"img_sim={r.get('image_sim')} img_norm={r.get('image_sim_norm')} "
                f"combined={r.get('final_score_combined')} | color={r.get('candidate_color')} "
                f"cat={it.get('main_category')}>{it.get('sub_category')}"
            )
